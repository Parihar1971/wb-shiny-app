...
# Filter for Nadia District
nadia <- wb_shapefile[wb_shapefile$NAME == "Nadia", ]
...
leaflet() %>%
  ...
  addPolygons(data = nadia,
              fillColor = "gray", weight = 2, color = "black", opacity = 1, fillOpacity = 0.6) %>%  # Adjust fillOpacity
  setView(lng = 88.45, lat = 23.2, zoom = 8)  # Adjust the map view to focus on Nadia
...
saveWidget(map, "nadia_map.html")

# Overlay a 6.6 kms x 6.6 kms grid on the map of Nadia District
...
# Create the bounding box from the Nadia shapefile
bbox <- st_bbox(nadia)

# Convert bbox to simple feature (geometry) using st_as_sfc
bbox_sf <- st_as_sfc(bbox)

# Create the grid over the bounding box of Nadia
grid <- st_make_grid(bbox_sf, cellsize = c(grid_size_deg, grid_size_deg), what = "polygons")

# Clip the grid to Nadia boundary using st_intersection
grid_clipped <- st_intersection(grid, nadia)
...
leaflet() %>%
  ...
  addPolygons(data = nadia, fillColor = "gray", weight = 2, color = "black", opacity = 1, fillOpacity = 0.1) %>%
  addPolygons(data = grid_clipped, color = "black", weight = 1, fillOpacity = 0) %>%  # Black gridlines
  setView(lng = 88.44, lat = 23.18, zoom = 10)  # Adjust the map view to focus on Nadia
...
# Save the map as an HTML file
saveWidget(map, "nadia_map_with_clipped_grid.html")

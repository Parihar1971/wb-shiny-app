...
# Filter for Murshidabad District
murshidabad <- wb_shapefile[wb_shapefile$NAME == "Murshidabad", ]
...
leaflet() %>%
  ...
  addPolygons(data = murshidabad,
              fillColor = "gray", weight = 2, color = "black", opacity = 1, fillOpacity = 0.6) %>%  # Adjust fillOpacity
  setView(lng = 88.3, lat = 24.2, zoom = 8)  # Adjust the map view to focus on Murshidabad
...
saveWidget(map, "murshidabad_map.html")

# Overlay a 6.6 kms x 6.6 kms grid on the map of Murshidabad District
...
# Create the bounding box from the Murshidabad shapefile
bbox <- st_bbox(murshidabad)

# Convert bbox to simple feature (geometry) using st_as_sfc
bbox_sf <- st_as_sfc(bbox)

# Create the grid over the bounding box of Murshidabad
grid <- st_make_grid(bbox_sf, cellsize = c(grid_size_deg, grid_size_deg), what = "polygons")

# Clip the grid to Murshidabad boundary using st_intersection
grid_clipped <- st_intersection(grid, murshidabad)
...
leaflet() %>%
  ...
  addPolygons(data = murshidabad, fillColor = "gray", weight = 2, color = "black", opacity = 1, fillOpacity = 0.1) %>%
  addPolygons(data = grid_clipped, color = "black", weight = 1, fillOpacity = 0) %>%  # Black gridlines
  setView(lng = 88.255, lat = 24.163, zoom = 8)  # Adjust the map view to focus on Murshidabad
...
# Save the map as an HTML file
saveWidget(map, "murshidabad_map_with_clipped_grid.html")
